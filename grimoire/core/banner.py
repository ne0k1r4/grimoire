# ═══════════════════════════════════════════════════════════════
#  GRIMOIRE v2.0 — core/banner.py
#
#  Developer  : Light
#  Alias      : Neok1ra
#  GitHub     : https://github.com/ne0k1r4
#  Tool       : GRIMOIRE — The Death Note of the digital world
# ═══════════════════════════════════════════════════════════════

GRIMOIRE_BANNER = r"""
  ██████╗ ██████╗ ██╗███╗   ███╗ ██████╗ ██╗██████╗ ███████╗
 ██╔════╝ ██╔══██╗██║████╗ ████║██╔═══██╗██║██╔══██╗██╔════╝
 ██║  ███╗██████╔╝██║██╔████╔██║██║   ██║██║██████╔╝█████╗  
 ██║   ██║██╔══██╗██║██║╚██╔╝██║██║   ██║██║██╔══██╗██╔══╝  
 ╚██████╔╝██║  ██║██║██║ ╚═╝ ██║╚██████╔╝██║██║  ██║███████╗
  ╚═════╝ ╚═╝  ╚═╝╚═╝╚═╝     ╚═╝ ╚═════╝ ╚═╝╚═╝  ╚═╝╚══════╝
"""

GRIMOIRE_BANNER_SMALL = r"""
 ░██████╗░██████╗░██╗███╗░░░███╗░█████╗░██╗██████╗░███████╗
 ██╔════╝░██╔══██╗██║████╗░████║██╔══██╗██║██╔══██╗██╔════╝
 ██║░░██╗░██████╔╝██║██╔████╔██║██║░░██║██║██████╔╝█████╗░░
 ██║░░╚██╗██╔══██╗██║██║╚██╔╝██║██║░░██║██║██╔══██╗██╔══╝░░
 ╚██████╔╝██║░░██║██║██║░╚═╝░██║╚█████╔╝██║██║░░██║███████╗
 ░╚═════╝░╚═╝░░╚═╝╚═╝╚═╝░░░░╚═╝░╚════╝░╚═╝╚═╝░░╚═╝╚══════╝"""

RYUK = (
    "         .  .\n"
    "        /|  |\\\n"
    "       / |  | \\\n"
    "      /  |  |  \\    I'm not doing this\n"
    "     | R | Y | U |   out of goodwill.\n"
    "     | K |   |   |   I just find humans\n"
    "      \\  |   |  /    interesting.\n"
    "       \\ |   | /\n"
    "        \\|___|/\n"
    "         |   |\n"
)

KIRA_QUOTES = [
    "I will write your name in this book.",
    "I am justice. I am the God of this new world.",
    "The world is rotten. Someone has to do this.",
    "From here, I will change the world.",
    "You can't win against the power of a god.",
    "No matter who they are, I won't lose to them.",
    "Once I sat down and thought, it was obvious.",
    "This is exactly as I planned.",
    "I'll solve equations with my right hand and write names with my left.",
    "The real evil is the power to kill people.",
]

KIRA_MOTD = [
    "The notebook is open. Begin.",
    "Names have power. Choose wisely.",
    "Kira sees everything.",
    "Justice is swift. So is this tool.",
    "Every target has a name. Write it down.",
    "The investigation ends here.",
    "I am the God of this new world.",
    "Checkmate.",
]

VERSION   = "2.0.0"
AUTHOR    = "Light"
ALIAS     = "Neok1ra"
GITHUB    = "https://github.com/ne0k1r4"
TAGLINE   = "The Death Note of the digital world"
TOOL      = "GRIMOIRE"
